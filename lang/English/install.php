...
'Install intro'				=>	'In order to install PanBB you must complete the form set out below. Please read the instructions provided before completing the form. If you encounter any difficulties with the installation, please refer to the documentation supplied as part of PanBB\'s download package.',
'Part1'						=>	'Part 1 — Database setup',
'Part1 intro'				=>	'Please enter the requested information in order to setup your database for PanBB. You must know all the information asked for before proceeding with the installation. Help with completing this part of the form is set out below.',
...
'Database type info'		=>	'PanBB currently supports MySQL, PostgreSQL and SQLite. If your database of choice is missing from the drop-down menu below, it means this PHP environment does not have support for that particular database. More information regarding support for particular versions of each database can be found in the FAQ.',
'Mysql type info'			=>	'PanBB has detected that your PHP environment supports two different ways of communicating with MySQL. The two options are called "<em>standard</em>" and "<em>improved</em>". If you are uncertain which one to use, start by trying improved and if that fails, try standard.',
...
'Database name info'		=>	'Enter the name of the database that PanBB will be installed into. The database must exist. For SQLite, this is the relative path to the database file. If the SQLite database file does not exist, PanBB will attempt to create it.',
'Database username info'	=>	'Enter the username and password used for connecting to the selected database. Ignore for SQLite.',
'Table prefix info'			=>	'Optional — enter a database table prefix. By specifying a table prefix you can run multiple copies of PanBB in the same database (example: <em>foo_</em>).',
...
'Database name help'		=>	'Existing database PanBB will be installed to.',
...
'Part2 intro'				=>	'Please enter the requested information in order to setup an administrator account for your PanBB installation. You can create more administrators and moderators later.',
...
'Base URL'					=>	'Base URL',
'Base URL info'				=>	'Please pay particular attention when entering your Base URL. You must set the correct Base URL or your forum will not work properly. The Base URL is the URL (without trailing slash) of your PanBB forum (example: <em>http://forum.example.com</em> or <em>http://example.com/~myuser</em>). Please note that the preset value below is just an educated guess by PanBB.',
'Base URL help'				=>	'The URL (without trailing slash) of your PanBB installation.',
...
// Install errors
'No database support'		=>	'This PHP environment does not have support for any of the databases that PanBB supports. PHP needs to have support for either MySQL, PostgreSQL or SQLite in order for PanBB to be installed.',
...
'Invalid MySQL version'		=>	'You are running MySQL version %1$s. PanBB requires at least MySQL %2$s to run properly. You must upgrade your MySQL installation before you can continue.',
...
'PunBB already installed'	=>	'A table called "%1$susers" is already present in the database "%2$s". This could mean that PanBB is already installed or that another piece of software is installed and is occupying one or more of the table names PanBB requires. If you want to install multiple copies of PanBB in the same database, you must choose a different table prefix.',
...
'Default post contents'		=>	'If you are looking at this (which I guess you are), the install of PanBB appears to have worked! Now log in and head over to the administration control panel to configure your forum.',
'Default rank 1'			=>	'New member',
'Default rank 2'			=>	'Member',


// Installation completed form
'Success description'		=>	'Congratulations! PanBB %s has been successfully installed.',
'Final instructions'		=>	'Final instructions',
'No write info 1'			=>	'Important! To finalize the installation, you need to click on the button below to download a file called config.php. You then need to upload this file to the root directory of your PanBB installation.',
'No write info 2'			=>	'Once you have uploaded config.php, PanBB will be fully installed! You may then %s once config.php has been uploaded.',
'Go to index'				=>	'go to the forum index',
'Warning'					=>	'Warning!',
'No cache write'			=>	'<strong>The cache directory is currently not writable!</strong> In order for PanBB to function properly, the directory named <em>cache</em> must be writable by PHP. Use chmod to set the appropriate directory permissions. If in doubt, chmod to 0777.',
...
'Write info'				=>	'PanBB has been fully installed! You may now %s.',
);
